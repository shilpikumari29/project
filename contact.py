...
def SubmitData():
    ...
    cursor = conn.cursor()
    cursor.execute("INSERT INTO `member` (firstname, lastname, gender, age, address, contact) VALUES (%s, %s, %s, %s, %s, %s)", (firstName, lastName, gender, age, address, contact))
    conn.commit()
    cursor.execute("SELECT * FROM `member` ORDER BY `lastname` ASC")
    fetch = cursor.fetchall()
    for data in fetch:
...
def DeleteData():
    if not tree.selection():
       result = tkMessageBox.showwarning('', 'List,Select kr Lo Pahle!', icon="warning")
    else:
